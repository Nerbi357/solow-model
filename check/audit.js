...
(async () => {
  ...
  const res = await p.evaluate(() => {
    ...
    /* Область перебора — вход, а не ожидание: слепота проверки в том, что модель
       ниже выписана заново, а не в том, что границы угаданы. У s она ниже 0,01:
       модели нужно лишь s > 0, см. «Диапазоны параметров» в CLAUDE.md. */
    const RNG  = {s:[0.0005,0.99], d:[0.01,0.99], n:[0,0.99], g:[0,0.99], a:[0.01,0.99]};
    const VS   = ['y','k','c','i'];
